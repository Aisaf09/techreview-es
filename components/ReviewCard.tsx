...
export default function ReviewCard({ frontmatter, slug, readingTime, priority = false }: ReviewCardProps) {
  ...
  return (
    <Link
      href={`/reviews/${slug}`}
      className="group flex flex-col overflow-hidden rounded-3xl bg-white card-shadow hover:card-shadow-hover transition-all duration-300 hover:-translate-y-1"
    >
      {/* Image */}
      <div className="relative aspect-[16/9] overflow-hidden bg-gray-100">
        ...
        />
        {/* Category badge */}
        <span className="absolute top-3 left-3 bg-white/90 backdrop-blur-sm text-brand-700 text-xs font-bold px-2.5 py-1 rounded-full shadow-sm">
          {CATEGORY_LABELS[frontmatter.category]}
        </span>
        {/* Rating overlay */}
        <span className="absolute top-3 right-3 bg-gray-900/80 backdrop-blur-sm text-amber-400 text-xs font-black px-2.5 py-1 rounded-full">
          ★ {frontmatter.rating.toFixed(1)}
        </span>
      </div>
      ...
      <div className="flex flex-col flex-1 p-5">
        <div className="flex items-center gap-2 mb-2">
          <span className="text-xs font-medium text-gray-400">{frontmatter.brand}</span>
          {readingTime && (
            <>
        ...
        <div className="flex items-center justify-between pt-3 border-t border-gray-100">
          <div>
            ...
            <p className="font-black text-brand-700 text-sm">{formatPrice(minPrice)}</p>
          </div>
